...
const CATEGORIES = {
  casa: { name: 'Casa', icon: '\uDD\uD83C\uDFE0', color: '#f87171' },
  cuerpo: { name: 'Cuerpo', icon: '\uDD\uD83D\uDCAA', color: '#4ade80' },
  gestiones: { name: 'Gestiones', icon: '\uDD\uD83D\uDCCB', color: '#fbbf24' },
  social: { name: 'Social', icon: '\uDD\uD83D\uDC65', color: '#60a5fa' },
  personal: { name: 'Personal', icon: '\uDD\uD83C\uDF1F', color: '#a78bfa' }
};
...
function renderCharacter() {
  ...
  document.getElementById('char-class-icon').textContent = cls ? cls.icon : '\uDD\uD83E\uDDD1‍\uDD\uD83C\uDF3E';
  ...
  document.getElementById('char-resources').innerHTML = `
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
      ...
      <div style="text-align: center;">
        <div style="font-size: 24px; color: var(--blue);">\uDD\uD83D\uDCA7 ${resources.mp}</div>
      ...
      <div style="text-align: center;">
        <div style="font-size: 24px; color: var(--purple);">\uDD\uD83C\uDFAF ${resources.focusMax}</div>
...
function renderEquipment() {
  ...
  const slotConfig = [
    ...
    { key: 'armor', name: 'Armadura', icon: '\uDD\uD83D\uDEE1️' },
    { key: 'accessory1', name: 'Accesorio 1', icon: '\uDD\uD83D\uDC8D' },
    { key: 'accessory2', name: 'Accesorio 2', icon: '\uDD\uD83D\uDC8D' },
    { key: 'artifact', name: 'Artefacto', icon: '\uDD\uD83D\uDD2E' }
  ];
...
function showLegacyItemModal(slotIndex) {
  ...
  actionBtn.textContent = used ? 'Recuperación ya usada' : '\uDD\uD83D\uDD04 Rehacer recompensa';
...
function renderSettings() {
  ...
  content.innerHTML = `
    ...
    <div class="card">
      ...
      <button class="btn btn-secondary mb-8" onclick="exportData()">\uDD\uD83D\uDCE4 Exportar save</button>
      <button class="btn btn-secondary mb-8" onclick="showImportModal()">\uDD\uD83D\uDCE5 Importar save</button>
      <button class="btn btn-ghost" onclick="resetGame()" style="color: var(--red)">\uDD\uD83D\uDDD1️ Resetear progreso</button>
    </div>
    ...
    <div class="card">
      ...
      <button class="btn btn-gold" onclick="exportSnapshot()">\uDD\uD83D\uDCCA Exportar Snapshot para Agente</button>
    </div>
...
function completeTask() {
  ...
  // Icon & title
  document.getElementById('complete-icon').textContent = currentIsOverflow ? '⚡' : '\uDD\uD83C\uDFC6';
...
function finalizeCompletion(sideQuestCompleted) {
  ...
  rewardsHtml += `<div class="complete-reward">+${goldEarned} \uDD\uD83E\uDE99</div>`;
  document.getElementById('complete-rewards').innerHTML = rewardsHtml;
  
  if (leveledUp) {
    document.getElementById('complete-title').textContent = '¡Subiste de nivel!';
    document.getElementById('complete-icon').textContent = '\uDD\uD83C\uDF89';
  }
...
function showCombatVictory() {
  ...
  document.getElementById('combat-result-icon').textContent = '\uDD\uD83C\uDFC6';
  ...
  if (rewards) {
    rewardsHtml = `
      ...
      <div class="complete-reward">+${rewards.gold} \uDD\uD83E\uDE99</div>
    `;
    if (rewards.drops && rewards.drops.length > 0) {
      for (const drop of rewards.drops) {
        const item = typeof ITEMS !== 'undefined' ? ITEMS[drop] : null;
        rewardsHtml += `<div class="complete-reward" style="color: var(--purple);">\uDD\uD83C\uDF81 ${item?.name || drop}</div>`;
      }
    }
  }
...
function showSavedTasks() {
  ...
  document.getElementById('modal-tasks-title').textContent = '\uDD\uD83D\uDCCC Tareas guardadas';
  ...
  if (navigator.share && navigator.canShare) {
    ...
    if (navigator.canShare({ files: [file] })) {
      navigator.share({
        ...
        text: `\uDD\uD83C\uDFAE Actualización de ${gameState.name} en ${gameState.guildName}`,
        files: [file]
...
function downloadReceipt(data, receipt) {
  ...
  alert(`Recibo generado!\\n\\nCompártelo con tu guild por WhatsApp o donde prefieras.\\n\\n\uDD\uD83D\uDCCA ${receipt.recentAchievements.tasksCompleted} tareas | +${receipt.recentAchievements.xpEarned} XP`);
}
  ...
  if (navigator.share && navigator.canShare) {
    ...
    if (navigator.canShare({ files: [file] })) {
      navigator.share({
        ...
        text: `\uDD\uD83C\uDFAE ¡Únete a mi guild "${gameState.guildName}" en LifeXP!`,
        files: [file]
...
function processReceipt(receipt) {
  ...
  if (receipt.type === 'progress_update') {
    ...
    alert(`Recibo de ${receipt.playerName} procesado!\\n\\n\uDD\uD83D\uDCCA Nivel ${receipt.currentState.level} | ${receipt.recentAchievements.tasksCompleted} tareas recientes`);
    renderGuild();
    return;
  }
...
function renderGuild() {
  ...
  if (!gameState.guildId) {
    container.innerHTML = `
      <div class="card" style="text-align: center; padding: 24px;">
        ...
        <button class="btn btn-gold mb-8" onclick="exportGuildInvite()">\uDD\uD83C\uDFF0 Crear Guild</button>
        <button class="btn btn-secondary" onclick="importReceipt()">\uDD\uD83D\uDCE5 Unirme con invitación</button>
      </div>
    `;
    return;
  }
  ...
  sorted.forEach((m, idx) => {
    ...
    const medal = idx === 0 ? '\uDD\uD83E\uDD47' : idx === 1 ? '\uDD\uD83E\uDD48' : idx === 2 ? '\uDD\uD83E\uDD49' : '▪️';
    const classIcon = typeof CLASS_TREE !== 'undefined' && CLASS_TREE[m.classId] 
      ? CLASS_TREE[m.classId].icon : '\uDD\uD83E\uDDD1‍\uDD\uD83C\uDF3E';
      ...
      <div class="card" style="display: flex; align-items: center; gap: 12px; ${isMe ? 'border-color: var(--gold);' : ''}">
        ...
        <div style="flex: 1;">
          ...
          <div style="font-size: 12px; color: var(--text-muted);">
            Lv ${m.level} · ${m.className || 'Novato'} · \uDD\uD83D\uDD25${m.streak || 0}
          </div>
        </div>
  ...
  container.innerHTML = `
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
      <button class="btn btn-gold" onclick="exportReceipt()">\uDD\uD83D\uDCE4 Enviar recibo</button>
      <button class="btn btn-secondary" onclick="importReceipt()">\uDD\uD83D\uDCE5 Recibir recibo</button>
    </div>
    <button class="btn btn-ghost" style="width: 100%; margin-top: 8px;" onclick="exportGuildInvite()">\uDD\uD83D\uDD17 Invitar a alguien</button>
  `;
}
...
function renderQuests() {
  ...
  if (active.length === 0) {
    container.innerHTML = `
      <div class="card" style="text-align: center; padding: 24px;">
        <div style="font-size: 32px; margin-bottom: 12px;">\uDD\uD83D\uDCDC</div>
    ...
    container.innerHTML += `
      <div class="card" onclick="showQuestDetail('${questState.questId}')" style="cursor: pointer; border-left: 3px solid ${color};">
        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
          ...
          <div style="text-align: right;">
            <div style="font-size: 20px;">${quest.icon || '\uDD\uD83D\uDCDC'}</div>
          </div>
        </div>
...
function showAvailableQuests() {
  ...
  title.textContent = '\uDD\uD83D\uDCDC Quests disponibles';
  ...
  const typeConfig = {
    daily:       { color: 'var(--green)',  label: 'Diaria',    icon: '\uDD\uD83D\uDCC5' },
    simple:      { color: 'var(--blue)',   label: 'Misión',    icon: '\uDD\uD83D\uDCDC' },
    compound:    { color: 'var(--purple)', label: 'Compuesta', icon: '\uDD\uD83D\uDCDA' },
    story:       { color: 'var(--gold)',   label: 'Historia',  icon: '⭐' },
    bounty:      { color: 'var(--red)',    label: 'Bounty',    icon: '\uDD\uD83C\uDFAF' },
    class_quest: { color: 'var(--cyan)',   label: 'Clase',     icon: '⚔️' },
    event:       { color: 'var(--orange)', label: 'Evento',    icon: '\uDD\uD83C\uDF89' },
  };
  ...
  for (const [questId, quest] of Object.entries(QUESTS)) {
    ...
    const cfg   = typeConfig[quest.type] || { color: 'var(--text-muted)', label: quest.type, icon: '\uDD\uD83D\uDCDC' };
    ...
    list.innerHTML += `
      <div class="card" style="cursor:pointer;border-left:3px solid ${cfg.color};margin-bottom:8px;" onclick="acceptQuest('${questId}')">
        ...
        <div style="display:flex;gap:10px;margin-top:8px;padding-top:6px;border-top:1px solid var(--border);">
          ...
          ${rewardGold ? `<span style="font-size:11px;color:var(--gold);">+${rewardGold} \uDD\uD83E\uDE99</span>` : ''}
...
function showQuestDetail(questId) {
  ...
  content.innerHTML = `
    <div style="text-align: center; margin-bottom: 16px;">
      <div style="font-size: 48px;">${quest.icon || '\uDD\uD83D\uDCDC'}</div>
    ...
    <div style="font-size: 12px; color: var(--gold);">
      Recompensa: +${quest.rewards?.xp || 0} XP | +${quest.rewards?.gold || 0} \uDD\uD83E\uDE99
    </div>
  `;
...
function showItemModal(itemId, container) {
  ...
  html += '<div style="font-size:48px;line-height:1;margin-bottom:6px;">' + (item.icon || '\uDD\uD83D\uDCE6') + '</div>';
  ...
  if (knownEffects.length) {
    ...
    knownEffects.forEach(function(e) {
      ...
      if (unlocked) {
        ...
        html += '<div style="font-size:12px;margin-bottom:4px;color:var(--text-muted);opacity:.5;">\uDD\uD83D\uDD12 ' + escapeItemHtml(e.name || 'Efecto');
  ...
  // -- VALUE -----------------------------------------------------------------
  html += '<div style="font-size:11px;color:var(--text-muted);margin-top:10px;text-align:right;">' + (item.value || 0) + ' \uDD\uD83E\uDE99</div>';
