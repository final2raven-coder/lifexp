...
function normalizeContainerName(container) {
  return container === 'stash' ? 'stash' : container === 'inventory' ? 'inventory' : null;
}

function getContainerList(container) {
  const normalized = normalizeContainerName(container);
  if (!normalized) return null;
  if (!Array.isArray(gameState[normalized])) gameState[normalized] = [];
  return gameState[normalized];
}

function normalizeContainerQuantity(quantity) {
  const value = Number(quantity);
  return Number.isInteger(value) && value > 0 ? value : null;
}

function getReservedQuantityForContainer(itemId, container, operationId = null) {
  if (typeof window !== 'undefined' && window.LifeXPMaterialInteractions && typeof window.LifeXPMaterialInteractions.getReservedQuantity === 'function') {
    return window.LifeXPMaterialInteractions.getReservedQuantity(itemId, container, operationId);
  }
  return 0;
}

function canRemoveFromContainer(itemId, container, quantity, operationId = null) {
  const normalizedQuantity = normalizeContainerQuantity(quantity);
  const list = getContainerList(container);
  if (!normalizedQuantity || !list) return { success: false, reason: 'invalid_quantity_or_container' };
  const slot = list.find(entry => entry && entry.id === itemId);
  const held = Number(slot?.qty || 1);
  const reserved = getReservedQuantityForContainer(itemId, container, operationId);
  if (!slot || held < normalizedQuantity + reserved) {
    return { success: false, reason: reserved > 0 ? 'reserved' : 'insufficient_quantity' };
  }
  return { success: true, held, reserved };
}
...
function addToContainer(itemId, container = 'inventory', quantity = 1) {
  ...
  if (item.type === 'consumable' || item.type === 'material') {
    ...
    if (existing) {
      existing.qty = (existing.qty || 1) + normalizedQuantity;
      return { success: true, stacked: true };
    }
  }
  list.push({ id: itemId, qty: normalizedQuantity });
...
function removeFromContainer(itemId, container = 'inventory', quantity = 1, options = {}) {
  const check = canRemoveFromContainer(itemId, container, quantity, options.operationId || null);
  if (!check.success) return check;
  const list = getContainerList(container);
  const index = list.findIndex(entry => entry && entry.id === itemId);
  const slot = list[index];
  slot.qty = (slot.qty || 1) - Number(quantity);
  if (slot.qty <= 0) list.splice(index, 1);
  return { success: true, removed: Number(quantity) };
}

function removeFromInventory(itemId, quantity = 1, options = {}) {
  return removeFromContainer(itemId, 'inventory', quantity, options).success;
}

function moveBetweenContainers(itemId, fromContainer = 'inventory', toContainer = 'stash', quantity = null) {
  const from = normalizeContainerName(fromContainer);
  const to = normalizeContainerName(toContainer);
  const source = getContainerList(from);
  const target = getContainerList(to);
  const slot = source?.find(entry => entry && entry.id === itemId);
  const requested = quantity === null ? Number(slot?.qty || 1) : normalizeContainerQuantity(quantity);
  if (!source || !target || !slot || !requested || from === to) return false;
  const check = canRemoveFromContainer(itemId, from, requested);
  if (!check.success) return false;
  if (!containerHasSpace(to, itemId) && !target.some(entry => entry && entry.id === itemId)) return false;
  const existing = target.find(entry => entry && entry.id === itemId);
  if (existing) existing.qty = (existing.qty || 1) + requested;
  else target.push({ id: itemId, qty: requested });
  slot.qty = (slot.qty || 1) - requested;
