...
function ensureCategoryTaskScreen() {
  ...
  screen.innerHTML = `
    <div class="header">
      <button class="btn btn-ghost btn-small" type="button" data-category-action="back" style="width: auto;">${LifeXPIcons.renderUI('ui.back', { size: 16 })} Back</button>
      <div id="category-task-heading" style="flex: 1; text-align: right;"></div>
    </div>
    <div class="content">
      <div id="category-task-summary" style="margin-bottom: 16px;"></div>
      <button class="btn btn-primary" type="button" data-category-action="random" id="category-task-random" style="margin-bottom: 18px;">${LifeXPIcons.renderUI('world.dice', { size: 20 })} Choose a task from this category</button>
...
function renderCategoryTaskList(catId) {
  ...
  heading.innerHTML = `${LifeXPIcons.renderCategory({ id: catId, name: presentation.getCategoryLabel(catId) }, { size: 28 })} ${presentation.getCategoryLabel(catId)}`;
  summary.innerHTML = `
    <div class="stat-card">
  ...
  const resultIcon = result.status === 'awaiting_side_quest'
    ...
    : (result.leveledUp
      ...
      : (result.isOverflow
        ? LifeXPIcons.renderUI('world.lightning', { size: 64 })
        : LifeXPIcons.renderUI('ui.trophy', { size: 64 })));
  document.getElementById('complete-icon').innerHTML = resultIcon;
