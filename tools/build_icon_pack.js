...
function candidateScore(reference, svgEntry) {
  const wanted = normalizedToken(reference.split('.').pop());
  const basename = normalizedToken(path.posix.basename(svgEntry, '.svg'));
  if (!wanted || !basename) return 0;
  if (wanted === basename) return 100;
  if (basename === `${wanted}icon` || basename === `${wanted}symbols`) return 90;
  if (basename.includes(wanted) || wanted.includes(basename)) return 60;
  return 0;
}
