...
function showOverflowTasks() {
  const list = document.getElementById('modal-tasks-list');
  document.getElementById('modal-tasks-title').innerHTML = `${LifeXPIcons.renderUI('world.lightning', { size: 18 })} Overflow tasks`;
    ...
    list.innerHTML += `
      <div class="card" style="cursor: pointer;" onclick="openOverflowTask('${task.id}')">
        <div style="display: flex; align-items: center; gap: 10px;">
          ...
          <div>
            <div style="font-weight: 600;">${task.name}</div>
            <div style="font-size: 12px; color: var(--text-muted);">${cat.name} · ${LifeXPIcons.renderUI('world.lightning', { size: 14 })} Overflow</div>
          </div>
        </div>
      </div>
    `;
